...
async function fetchProductList(url) {
  ...
  for (let i = 0; i < packSize / quantWords; i++) {
    ...
    await page.waitForSelector('label[for="nouns"]');
    await page.click('label[for="nouns"]');
    ...
    await page.waitForTimeout(wordDelay);
  ...
  fs.readFile("dictionaryHard.txt", "utf8", (err, data) => {
    ...
    fs.writeFile("dictionaryHard.txt", newWordList.join(","), () => {
